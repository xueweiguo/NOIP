...
long long leaveFromTree(int row, int col) {
	...
	if (found != 0) {
	    ...
	    if (offset == 0) {
			pair.first = getNode();
			init(pair.first, node[found].key + 1, length, node[found].key + 1);
		}
		else if(offset == node[found].length - 1){
			pair.first = getNode();
			init(pair.first, node[found].key, length, node[found].key);
		}
		else {
			pair.first = getNode();
			init(pair.first, node[found].key, offset, node[found].key);
			pair.second = getNode();
			init(pair.second, node[found].key + offset + 1, length - offset, node[found].key + offset + 1);
		}
		root[row] = erase(root[row], node[found].key);
		if (pair.first != 0) {
